...
> **用途**：每次新开 AI session 时读取此文件，恢复机器登录、目录结构、脚本用法等上下文。
> **最后更新**：2025-07
...
```bash
# Orin: 同步 C++ 源码 + 重新编译
rsync -avz cpp_infer/src/ -e "sshpass -p '0' ssh -o StrictHostKeyChecking=no" \
  dog@30.78.35.104:/data/wy/wall-x/cpp_infer/src/

# Orin: 同步 Python 脚本
rsync -avz scripts/ -e "sshpass -p '0' ssh -o StrictHostKeyChecking=no" \
  dog@30.78.35.104:/data/wy/wall-x/scripts/

# 5090: 同步整个项目
rsync -avz --exclude '.git' --exclude '__pycache__' --exclude '*.egg-info' \
  --exclude 'build' --exclude 'dist' \
  -e "sshpass -p ubuntu ssh -o StrictHostKeyChecking=no" \
  /Users/sam/project/github/wall-x/ ubuntu@30.79.32.148:/home/ubuntu/project/wall-x/
```
...
### Flow Action (Orin, bf16, batch=1, 488 tokens, 5步 ODE)
