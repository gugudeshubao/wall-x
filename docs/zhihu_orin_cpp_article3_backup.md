...
- **C++ VQA 文本生成实测：3469ms / 18.45 tok/s**（64 tokens），比 Python FA2（6360ms / 10.1 tok/s）快 **1.83x**
- VQA 每步 decode 49.0ms（vs Python 97.2ms），框架开销从 67.5ms 降至 ~19ms——但逐 token GPU 同步（argmax → `.item()`）成为新瓶颈
...
### 4.0 wall-x 的推理架构：ViT 是共享前端

在讨论 C++ 重写方案之前，先明确 wall-x 的推理架构。ViT（Vision Transformer，32 层）是 VQA 和 Flow Action **共享的图像编码前端**：
...
ViT 内部的算子（VisionAttention、VisionMLP、VisionRMSNorm）和 LLM decoder 层结构类似，但参数独立。第一、二篇的 VQA 测试使用真实图片输入，走了完整的 ViT 编码。本篇 C++ 重写覆盖了 **ViT + Transformer + MoE + ODE 全管线**，VQA 和 Flow Action 的 ViT 部分完全复用。ViT 目前仍为 bf16，后续做 INT8 量化后两种任务都会受益。

两种任务虽然共享 ViT 前端和 Transformer backbone，但后端推理模式差异很大：
...
| **延迟指标** | tok/s | Hz（控制频率） |
| **实时性要求** | 低（对话场景，秒级可接受） | 高（≥2 Hz 控制循环，500ms 硬约束） |
| **框架开销影响** | 大（64-128 步 × 67.5ms/步 = 4,320ms+） | 大（5 步 × 67.5ms/步 = 337ms） |
| **C++ 改造必要性** | 中（开销可容忍但浪费） | **极高**（67% 空转直接卡死控制频率） |

这张表解释了为什么系列从第三篇开始切换到 Flow Action 作为 benchmark 目标——VQA 的秒级延迟在对话场景可以接受，但 Flow Action 的 500ms 硬约束让框架开销变成了不可忽视的瓶颈。C++ 改造的动机正是来自这个实时性需求。
...
### 8.9 C++ VQA 文本生成：补齐最后一块拼图

前面展示的都是 Flow Action 的 C++ 加速。VQA（文本生成）同样受框架开销困扰——第二篇的分析显示 **64 步 decode 累积了 ~4320ms 的框架空转**（67.5ms/步 × 64 步）。C++ 引擎新增了 `generate_text()` 方法，实现了完整的 autoregressive decode loop（prefill → lm_head → argmax → embed → forward → 重复），下面是实测结果。

**测试条件**：Dummy 输入（200 text + 256 image tokens = 456 tokens），生成 64 个 token，warmup 3 次 + benchmark 10 次。
...
**VQA decode 的 19ms 残余开销来自逐 token GPU 同步**：autoregressive 生成每步必须调用 `argmax().item<int64_t>()` 把 token ID 从 GPU 拷回 CPU，这是一次阻塞式的 `cudaDeviceSynchronize()`。ODE 积分没有这个问题——5 步循环全在 GPU 端执行，不需要中间结果回传。

#### VQA vs Flow Action：同一引擎，6.3 倍差距的来源
...
```
Flow Action:  5 步 × 26.2 ms =  131 ms  ← 并行出结果，无需中间同步
VQA:         63 步 × 49.0 ms = 3086 ms  ← 串行蹦字，每步都等 GPU→CPU
```

**本质区别**：Flow Action 的 ODE 积分是"给 5 个时间步，GPU 连续跑完出动作"——中间结果（速度场）不需要回传 CPU。VQA 的 autoregressive decode 是"生成一个字，拿回来看看是不是结束符，再生成下一个"——**每步都有一次 GPU→CPU 的阻塞等待**。这不是工程能优化掉的，而是 autoregressive 解码的固有限制。

这也解释了一个直觉：**Flow Action 是 wall-x 在 Orin 上真正适合实时部署的任务**——5 步 ODE、无中间同步、框架开销清零后接近纯 GPU 时间。VQA 即使换了 C++ 引擎，decode 的串行本质依然让它比 Flow Action 慢一个数量级。

这指向了 VQA decode 的下一个优化方向——**speculative decoding 或 CUDA Graph capture**，通过批量化 decode 步骤减少 GPU↔CPU 同步次数。
...
| VQA 文本生成同样受益 | Python 6360ms → C++ 3469ms (1.83x) | ✓ 每步 decode 2.0x 加速 |
